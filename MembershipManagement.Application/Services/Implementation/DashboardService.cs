...
namespace MembershipManagement.Application.Services.Implementation
{
    public class DashboardService : IDashboardService
    {
        ...
        {
            var bookingData = _unitOfWork.Sealing.GetAll(u => u.AppliedByDate >= DateTime.Now.AddDays(-30) &&
             u.AppliedByDate.Date <= DateTime.Now)
                 .GroupBy(b => b.AppliedByDate.Date)
                 .Select(u => new {
                     DateTime = u.Key,
        ...
        //{
        //    var totalBookings = _unitOfWork.Sealing.GetAll(u => u.Status != SD.StatusPending
        //  || u.Status == SD.StatusCancelled);
